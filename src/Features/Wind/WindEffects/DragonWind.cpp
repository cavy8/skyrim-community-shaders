		...
		private:
				...
				const DragonWind::Settings& a_settings)
			{
				float3 horizontalDirection = Normalize(
					float3(a_state.filteredVelocity.x, a_state.filteredVelocity.y, 0.0f));
				if (LengthSquared(horizontalDirection) <= 1e-6f) {
					const auto aimDirection = Util::GetAimDirection(a_actor);
					horizontalDirection = Normalize(float3(aimDirection.x, aimDirection.y, 0.0f));
				}
				const float3 downwashDirection = Normalize(float3(horizontalDirection.x * kWingbeatHorizontalDrift,
