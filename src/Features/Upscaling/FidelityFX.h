...
public:
	...
	/**
	 * @brief Dispatches FSR upscaling for the current frame.
	 * @param a_upscalingTexture The input color texture to upscale; also the output target.
	 * @param a_depth Scene depth. Must be a typed format (R32_FLOAT) when the runtime
	 *  provider is active -- the DX11/DX12 shared-resource bridge cannot carry the game's
	 *  R24G8_TYPELESS depth allocation.
	 ...
	 */
	void Upscale(ID3D11Resource* a_upscalingTexture, ID3D11Resource* a_depth, ID3D11Resource* a_reactiveMask, ID3D11Resource* a_transparencyCompositionMask, ID3D11Resource* a_motionVectors, float a_sharpness);
	...
	/**
	 * @brief Upscales one context via the runtime provider or the host FSR3 SDK. Dispatch
	 * primitive used by Upscale(); chooses runtime vs host FSR3.
	 * @param a_contextIndex Context index; must be less than the active context count.
...
private:
	...
	// True if a runtime-provider (FSR4/runtime-FSR3) frame was already published this same
	// frame index -- a later dispatch must not fall through to the host FSR3 SDK, which
	// would present a mixed-provider result. See UpscaleRegion's failure paths.
	...
	// failure flag immediately (so it is not attempted again this frame) and marks it
