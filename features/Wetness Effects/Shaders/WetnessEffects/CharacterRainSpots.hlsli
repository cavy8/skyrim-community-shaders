...
namespace CharacterRainSpots
{
	...
	/** @brief Returns surface-attached coverage, water height, and trail absorption. */
		...
		bool heldWeapon)
	{
		...
		float3 water = float3(0.0f, 0.0f, 0.0f);
		{
			...
			// This fork has no stereo/VR camera array, so the head-relative position is
			// simply the position relative to the single camera.
			float3 headRelativePosition = relativeWorldPosition;
